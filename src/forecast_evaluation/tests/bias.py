...
def bias_analysis(
    ...
    df = data_filtered._main_table.assign(horizon=lambda d: d["target_minus_vintage"].astype(int))
    ...
    for i, (idx, row) in enumerate(combinations.iterrows()):
        ...
        # Run bias test
        result = evaluate_bias(df, variable, source, metric, horizon, verbose=verbose)
        ...
        results_list[i] = {
            ...
            "hac_maxlags": result.cov_kwds["maxlags"],
            "ci_lower": ci_lower,
            "ci_upper": ci_upper,
        }
